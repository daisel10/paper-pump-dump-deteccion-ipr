...
\begin{table}[htbp]
  ...
  \begin{tabular}{l l p{3.8cm}}
    ...
    \hline
    $T_{\text{cal}}$ & 720~h (30~d) & RMT válida con $T \gg N$ \\
    $T_{\text{det}}$ & 48~h & Duración típica de acumulación~\cite{lamorgia2020} \\
    $M$ & 10\,000 redes & ${\approx}30$ valores en la cola del percentil~99.7 \\
    Umbral & $\hat{F}_{\text{null}}^{-1}(0.997)$ & Percentil~99.7 empírico (no paramétrico) \\
    Sens.\ RMT & ${\pm}15\%$ en $\lambda_+$ & Cambio en precisión $< 5\%$ \\
    \hline
  \end{tabular}
\end{table}
